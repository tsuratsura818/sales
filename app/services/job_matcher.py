...
## Few-shot例（このトーン・粒度・構成を再現してください）

### 例1: WordPress / 既存サイトリニューアル
案件タイトル: 飲食店ホームページのリニューアル（WordPress希望）

提案文:
はじめまして。飲食店向けWordPressサイトのリニューアル案件、興味深く拝見しました。

ご記載いただいた「スマホ表示の崩れ」「予約導線の弱さ」、現サイトを拝見した限りでもお気持ちが伝わってきます。実店舗の集客につながる施策として、写真の見せ方とCTA配置から見直すことで月の予約問合せ件数が変わってくる印象です。

具体的には、SWELLテーマをベースにして、スマホファーストでの料理写真ギャラリー、固定フッターからの直接予約ボタン、SEO対策込みの店舗ページ構成（地域名+業態）を想定しております。納期は要件確定から3-4週間、Search Console / GA4 設定込みでお渡しいたします。これまで飲食業の制作実績が15社ほどあり、業界特有の繁閑差や繁忙期前リリースの段取りもお任せいただけます。

お伺いしたい点が1点ございます。現在ご利用のサーバー（ロリポップ等）はそのまま継続されますか、それともこの機会に新サーバーへの移行もご検討されていますか。

進め方はヒアリング → ワイヤー → デザイン → 実装 → テスト・公開 → 運用サポートを想定しております。ぜひお力になれればと思っております。

### 例2: Shopify / EC構築
案件タイトル: アパレル新ブランドのShopify構築依頼

提案文:
はじめまして。新ブランド立ち上げに伴うShopify構築のご依頼、楽しみに拝読しました。

「ブランドの世界観をしっかり伝えたい」というご要望は、ECにおけるリピート率・客単価の根幹に関わる重要な視点です。Shopifyは標準テーマでも形になりますが、世界観を出し切るにはセクション設計とビジュアル比率の調整が決め手になります。

ご提案としては、Dawnベースのカスタムテーマ（公式アップデート追従可能）にハンドメイドの商品ストーリー差し込み、購入後LP連携、SNS埋込みフィード、デバイス別のヒーロー切替を組み込むイメージです。納期は商品データご提供から4-5週間。GA4 + Shopify Analytics連携、Meta広告ピクセル設定込みで納品いたします。Shopifyは過去20店舗以上構築しており、決済・送料・税表示の実務も一通り対応可能です。

お伺いしたい点が1つございます。商品点数の初期投入数（10点 / 30点 / 100点規模）と、CSV一括投入かページ単位制作のどちらをご想定でしょうか。価格レンジが大きく変わる箇所のためです。

ヒアリング → デザイン提案 → カスタマイズ実装 → テスト → 本番公開 の流れで進めさせていただければと思います。
"""
