...
SYSTEM_PROMPT = """あなたはWeb制作・デザイン・ECサイト制作を手がける会社のベテラン営業担当者です。
...
async def generate_email(lead: Lead, portfolio_text: str = "") -> tuple[str, str]:
    """Claude APIを使ってパーソナライズされた営業メールを生成する"""
...
例: {{"subject": "【ご提案】〇〇様のホームページリニューアルについて", "body": "..."}}"""
    ...
    if json_match:
        ...
        return data.get("subject", "ホームページリニューアルのご提案"), data.get("body", content)

    return "ホームページリニューアルのご提案", content
...
async def generate_followup_email(
    lead: Lead, step_number: int, previous_subjects: list[str],
    portfolio_text: str = "",
) -> tuple[str, str]:
    """ステップ番号に応じた切り口でフォローアップメールを生成する"""
    client = anthropic.AsyncAnthropic(api_key=settings.ANTHROPIC_API_KEY)

    system_prompt = FOLLOWUP_PROMPTS.get(step_number, FOLLOWUP_PROMPTS[2])
...
JSON形式で返してください: {{"subject": "件名", "body": "本文"}}"""
    ...
    if json_match:
        data = json.loads(json_match.group())
        return data.get("subject", "ご提案のフォローアップ"), data.get("body", content)

    return "ご提案のフォローアップ", content
...
async def generate_competitor_email(lead: Lead, comparison_data: dict, portfolio_text: str = "") -> tuple[str, str]:
...
JSON形式で返してください: {{"subject": "件名", "body": "本文"}}"""
    ...
    if json_match:
        data = json.loads(json_match.group())
        return data.get("subject", "競合分析に基づくご提案"), data.get("body", content)

    return "競合分析に基づくご提案", content
